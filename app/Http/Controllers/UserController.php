...
{
    $candidatePayments = auth()->user()
        ->whereNotNull('package_id') // Ensure user has a package
        ->with('package') // Load package details
        ->whereHas('package') // Only fetch users with a valid package relationship
        ->orderBy('featured_package_start_at', 'DESC') // Order by package start date
        ->get();

    return view('user.payment_history', compact('candidatePayments'));
}
