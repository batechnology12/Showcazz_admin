...
class AdminCompanyController extends Controller
{
    ...
    {
        $query = Company::with(['industry', 'ownershipType', 'country', 'state', 'city']);
        ...
        foreach ($companies as $company) {
            ...
            $company->followers_count = FavouriteCompany::where('company_id', $company->id)->count();
    ...
    public function show($id)
    {
        ...
        $followers = FavouriteCompany::where('company_id', $company->id)
            ->with('user')
            ->orderBy('created_at', 'desc')
    ...
    public function exportData($id)
    {
        ...
        $filename = 'company_' . $company->id . '_' . Str::slug($company->name) . '_' . date('Y-m-d') . '.json';
    ...
    private function getDetailedAnalytics($companyId)
{
    ...
    $engagementByDay = collect($engagementByDay)->map(function($item) {
        ...
        $item->day_name = $days[(int)$item->day_of_week] ?? 'Unknown';
        return $item;
    });
    ...
    // Follower statistics
    $totalFollowers = FavouriteCompany::where('company_id', $companyId)->count();
    $newFollowersToday = FavouriteCompany::where('company_id', $companyId)
        ->whereDate('created_at', today())->count();
    $newFollowersThisWeek = FavouriteCompany::where('company_id', $companyId)
        ->where('created_at', '>=', $lastWeek)->count();
    $newFollowersThisMonth = FavouriteCompany::where('company_id', $companyId)
    ...
    // FIXED: Application statistics - Check which table exists
    $totalApplications = 0;
    ...
    private function getRecentActivity($companyId)
    {
        ...
        $recentPosts = Post::where('user_id', $companyId)
            ...
            ->map(function($post) {
                return [
                    ...
                    'url' => route('companies.post.show', $post->id),
                    'icon' => 'fa-file-text',
                    'color' => 'blue',
                ];
            });
        ...
        $recentLikes = PostLike::whereIn('post_id', function($query) use ($companyId) {
            ...
            ->map(function($like) {
                return [
                    'type' => 'like',
                    'description' => ($like->user ? $like->user->name : 'Someone') . ' liked post: ' . ($like->post ? $like->post->title : 'Unknown'),
                    'created_at' => $like->created_at,
                    'url' => route('companies.post.show', $like->post_id),
                    'icon' => 'fa-heart',
                    'color' => 'red',
                ];
            });
        ...
        $recentComments = PostComment::whereIn('post_id', function($query) use ($companyId) {
            ...
            ->map(function($comment) {
                return [
                    'type' => 'comment',
                    'description' => ($comment->user ? $comment->user->name : 'Someone') . ' commented on: ' . ($comment->post ? $comment->post->title : 'Unknown'),
                    'created_at' => $comment->created_at,
                    'url' => route('companies.post.show', $comment->post_id),
                    'icon' => 'fa-comment',
                    'color' => 'green',
                ];
            });
        ...
        // Recent followers
        $recentFollowers = FavouriteCompany::where('company_id', $companyId)
            ->with('user')
            ->latest()
            ->limit(10)
            ->get()
            ->map(function($follower) {
                return [
                    'type' => 'follower',
                    'description' => 'New follower: ' . ($follower->user ? $follower->user->name : 'Unknown User'),
