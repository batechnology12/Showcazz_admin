...
class AdminPostController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->filled('author')) {
            ...
            // Search in users (including companies since they're in same table)
            $users = User::where('first_name', 'LIKE', '%' . $request->author . '%')
                ->orWhere('last_name', 'LIKE', '%' . $request->author . '%')
                ->orWhere('company_name', 'LIKE', '%' . $request->author . '%')
        ...
        $stats = [
            ...
            'active' => Post::whereNotIn('category_id', [5, 6])->where('is_active', true)->count(),
            'inactive' => Post::whereNotIn('category_id', [5, 6])->where('is_active', false)->count(),
        ...
        foreach ($posts as $post) {
            ...
            $post->author_type = User::find($post->user_id)?->usertype ?? 'unknown';
        }
    ...
    public function show($id)
    {
        ...
        // Get author info
        $author = $this->getAuthorInfo($post->user_id);

        // Parse media - using helper method
        ...
        return view('admin.post_new.show', compact('post', 'author', 'images', 'files', 'stats'));
    }
    ...
    public function approvePost($id)
    {
        ...
        if ($user && $user->firebase_token && $user->push_notification) {
            $this->fcmService->fcmSendNotification(
                $user->firebase_token,
                'Post Approved!',
                'Your post "' . Str::limit($post->title, 50) . '" has been approved and is now live.',
                ['screen' => 'post_details', 'post_id' => (string)$post->id]
            );
        }
    ...
    public function rejectPost(Request $request, $id)
    {
        ...
        if ($user && $user->firebase_token && $user->push_notification) {
            $this->fcmService->fcmSendNotification(
                $user->firebase_token,
                'Post Rejected',
                'Your post "' . Str::limit($post->title, 50) . '" was rejected. Reason: ' . $request->reason,
                ['screen' => 'post_details', 'post_id' => (string)$post->id]
            );
        }
    ...
    public function toggleStatus($id)
    {
        ...
        if ($user && $user->firebase_token && $user->push_notification) {
            $this->fcmService->fcmSendNotification(
                $user->firebase_token,
                'Post ' . $statusText,
                'Your post "' . Str::limit($post->title, 50) . '" has been ' . strtolower($statusText) . ' by admin.',
                ['screen' => 'post_details', 'post_id' => (string)$post->id]
            );
        }
        ...
        return response()->json([
            'success' => true,
            'message' => 'Post ' . strtolower($statusText) . ' successfully',
    ...
    public function bulkAction(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'reason' => 'required_if:action,reject,delete|string|max:500',
        ]);
        ...
        foreach ($request->post_ids as $postId) {
            ...
            $user = User::find($post->user_id);
            $userEligible = $user && $user->firebase_token && $user->push_notification;
                ...
                case 'approve':
                    ...
                    if ($userEligible) {
                        $this->fcmService->fcmSendNotification(
                            $user->firebase_token,
                            'Post Approved!',
                            'Your post "' . Str::limit($post->title, 50) . '" has been approved.',
                            ['screen' => 'post_details', 'post_id' => (string)$post->id]
                        );
                    }
                    $count++;
                ...
                case 'reject':
                    ...
                    if ($userEligible) {
                        $this->fcmService->fcmSendNotification(
                            $user->firebase_token,
                            'Post Rejected',
                            'Your post "' . Str::limit($post->title, 50) . '" was rejected. Reason: ' . ($request->reason ?? 'Policy violation'),
                            ['screen' => 'post_details', 'post_id' => (string)$post->id]
                        );
                    }
                    $count++;
                ...
                case 'delete':
                    ...
                    if ($userEligible) {
                        $this->fcmService->fcmSendNotification(
                            $user->firebase_token,
                            'Post Deleted',
                            'Your post "' . Str::limit($post->title, 50) . '" has been deleted by admin. Reason: ' . ($request->reason ?? 'Admin action'),
                            ['screen' => 'home']
                        );
                    }
        ...
        try {
            ...
            if ($user && $user->firebase_token && $user->push_notification) {
                $this->fcmService->fcmSendNotification(
                    $user->firebase_token,
                    'Post Deleted',
                    'Your post "' . Str::limit($post->title, 50) . '" has been deleted by admin. Reason: ' . $request->reason,
                    ['screen' => 'home']
                );
            }
    ...
    public function getPostStats($id)
    {
        ...
        $likesTrend = PostLike::where('post_id', $id)
        ...
        $commentsTrend = PostComment::where('post_id', $id)
            ->select(DB::raw('DATE(created_at) as date'), DB::raw('COUNT(*) as count'))
            ->groupBy('date')
            ->orderBy('date', 'desc')
            ->limit(30)
